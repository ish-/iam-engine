...
void Renderer::render (shared_ptr<Camera> camera, shared_ptr<Light> light, shared_ptr<Mesh> mesh) {
  ...
  mat4 view = camera->getAbsTransformMatrix();
  mat4 modelView = view * mesh->getAbsTransformMatrix();
  mat4 modelViewProjection = camera->projection * modelView;
  mat4 normalMat = glm::inverseTranspose(modelView);

  glUseProgram(shId);
  glUniformMatrix4fv(glGetUniformLocation(shId, "model"), 1, GL_FALSE, value_ptr(mesh->getAbsTransformMatrix()));
  ...
  glUniform3fv(glGetUniformLocation(shId, "lightPos"), 1, value_ptr(light->getAbsTransformMatrix()));
}
